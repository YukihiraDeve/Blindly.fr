...
fetch('/Room_PHP/Game/stock.json')
...
function generator() {
    ...
    var x = Math.floor(Math.random() * tab.length);
...
function Init() {
    ...
    setInterval("generator()", 30000);
     setInterval("restart()", 30000);
     setInterval("DisplaySon()", 29999);
}
...
function restart() {
    ...
    document.getElementById("labelInput").innerHTML = "Donnez la réponse - SANS CARACTERE SPECIAUX ";
    ...
    if (round == localStorage.getItem("round")) {
        window.location.href = '/Room_PHP/Game/EndOfRoom/index.php';
...
function getValue() {
    ...
    if ((artiste.search(input != 1) && input == artiste && sessionStorage.getItem("bloque_artiste") != 1)) {
    ...
    if ((musique.search(input) != 1) && (input == musique) && sessionStorage.getItem("bloque_musique") != 1) {
